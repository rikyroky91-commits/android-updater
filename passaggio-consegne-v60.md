# Passaggio di consegne — v60 (2026-08-12)

## -8. La ricerca priorizza la variante europea, non quella con la build più recente
...
Per Samsung questo esisteva già (`_ORDINE_MERCATI_SAMSUNG`). Per
OnePlus/OPPO no: il tracker ARB restituisce una riga per regione per lo
stesso telefono («OnePlus 13» = `CPH2653` in Europa/Global, `CPH2649`
in India), ordinate per build più recente — con la fixture di test
l'India aveva la build più recente e quindi vinceva sempre lei come
risultato principale della ricerca, esattamente il "modello a caso"
segnalato.

**Fix**: `_rango_regione_arb()` in `core/sources.py` — Europa prima,
poi Global, poi le altre regioni nell'ordine di prima (build più
recente). Tutte le regioni restano nel risultato (l'informazione
multi-regione è il valore di questa fonte), cambia solo quale diventa
il risultato principale mostrato in testa.

Verificato con uno script end-to-end (non solo letto sul codice) che il
riordino successivo per data in `core/scan.py::search_model` non
cancella questo ordine: `now_iso()` tronca ai secondi, quindi tutte le
righe della stessa ricerca hanno lo stesso timestamp e l'ordinamento
stabile di Python preserva l'ordine per regione fino alla pagina.

**Controllato ma non toccato**: il catalogo AER (realme, HONOR,
Motorola...) elenca già tutte le regioni di un modello nella STESSA
riga — verificato sul fixture vero, nessuna ambiguità da risolvere lì.
Xiaomi e le fonti non-AER di HONOR/Motorola/realme potrebbero avere lo
stesso problema, ma senza un fixture o un caso reale da verificare
sarebbe indovinare — lasciati per un giro futuro se emerge un caso
concreto.

Test nuovi: `tests/test_oplus_arb.py::TestFonteInSources` (+3).
Dettagli completi in `FONTI.md`.

## -7. Lo stato «Errore» non aveva via d'uscita: chi sbaglia un valore restava bloccato

Appena il v58 è arrivato in produzione, l'utente ha provato il nuovo
pulsante "Salva adesso" e ha ricevuto `Errore` — `GitHub ha risposto
401: {"message": "Bad credentials"}`. Screenshot del pannello
Environment di Render: `BACKUP_GIST_ID` e `BACKUP_GITHUB_TOKEN` avevano
lo stesso identico valore — un ID di Gist incollato per errore in
entrambe le variabili, invece di un vero token GitHub nella seconda.

Il token era davvero sbagliato — GitHub ha risposto correttamente. Il
problema era nell'interfaccia appena costruita al punto -6: lo stato
`Errore` mostrava SOLO "Salva adesso", che riprova con la STESSA
configurazione sbagliata e fallisce sempre allo stesso modo — nessun
modo di rifare la configurazione da quella pagina, esattamente il
vicolo cieco che "semplifica tutto" doveva evitare.

**Fix**: in `diagnostica.html`, lo stato `Errore` ora mostra ENTRAMBI i
moduli — "Salva adesso" (per riprovare, utile se l'errore era
temporaneo) e "Rifai la configurazione" (lo stesso modulo del punto -6,
titolo diverso, più una nota che nomina il sospetto più probabile —
valori scambiati o duplicati) per ripartire da un token nuovo senza
dover indovinare quale valore fosse sbagliato.
...
## -6. Configurare il backup da tre pagine a una

Segnalato dall'utente subito dopo aver ricevuto il v57 (il punto -5
sotto, la sezione di stato in Diagnostica): **«dice che non è
configurato nonostante abbia ascoltato le tue istruzioni. metti il
tool in diagnostica e semplifica tutto»**. La configurazione manuale
richiedeva tre passaggi su tre siti diversi (token GitHub, Gist creato
a mano, due valori incollati su Render) — tre occasioni di sbagliare
qualcosa, e nessun modo diretto di distinguere "ho sbagliato" da "ho
fatto tutto giusto ma Render non si è ancora riavviato".

**Fix**: `core/backup.py` aveva già tre funzioni scritte ma mai
collegate a nessuna route (`verifica_token`, `crea_archivio`,
`prova_completa`). Questo giro le collega a due nuove rotte in
`/diagnostica`:

* `POST /diagnostica/backup/crea` — incolli un token, la rotta lo
  verifica, crea l'archivio privato su Gist e ci scrive/rilegge un
  valore di prova, tutto in una chiamata. Compare in Diagnostica solo
  quando lo stato è "Non configurato".
* `POST /diagnostica/backup/salva` — pulsante "Salva adesso, per
  verificare", esegue `backup.salva()` in modo sincrono (a differenza
  di `_backup_subito()` del punto -3, volutamente fire-and-forget: qui
  è un'azione diagnostica esplicita, ha senso aspettare l'esito vero).

**Quello che resta manuale, e perché**: incollare i due valori nel
pannello Environment di Render non si può automatizzare da qui — non
c'è accesso alle API di Render, e richiederebbe un secondo insieme di
credenziali più privilegiate di quanto serva. Restano due passaggi
invece di sei, con l'esito di ognuno mostrato subito.

**Sicurezza**: il token non viene mai scritto su disco né loggato,
vive solo nella singola richiesta che lo verifica; campo
`type="password"`; non viene mai ririnviato nella pagina di risposta.

**Perché il tentativo manuale precedente mostrava ancora "Non
configurato"** (non verificabile da qui, due spiegazioni più
probabili): il v57 — la sezione «Backup esterno» stessa — potrebbe non
essere ancora applicato al repository reale, oppure le variabili sono
state salvate ma Render non ha ancora finito il riavvio automatico. Il
nuovo pulsante "Salva adesso" dà un modo diretto di controllare la
configurazione attuale invece di aspettare.

Test nuovi: `tests/test_sito.py::TestDiagnosticaConfigurazioneBackup`
(6). Dettagli completi in `FONTI.md`.

## -5. La pagina Diagnostica non diceva niente sul backup esterno

Segnalato dall'utente con screenshot della Diagnostica vera, subito
dopo aver ricevuto il fix del punto -3 sotto (il backup immediato a ogni
...
sono ancora impostati su Render — dal punto -6 sopra ora c'è un modulo
in pagina che li configura senza uscire da Diagnostica.
...
nessuna copia duratura da nessuna parte. Dal punto -6 sopra, il modulo
"Configura il backup" in Diagnostica fa questi due passaggi in uno.
...
**1028 test passati, 416 subtest passati, 0 falliti** (954 sulla base
ricevuta → 980 → 985 → 991 → 994 → 997 → 1013 → 1018 → 1024 → 1025 →
1028 nei giri successivi di questa sessione).
...
**Stesso principio vale ora per i punti -3/-5/-6**: se dopo questa
consegna il backup continua a sembrare "non configurato" nonostante i
valori sembrino giusti, il primo sospetto non è il codice — è (a) se
questo zip è già stato applicato al repository vero, e solo poi (b) se
Render ha finito di riavviarsi dopo aver salvato le variabili. Il
pulsante "Salva adesso" del punto -6 è pensato apposta per rispondere a
questa domanda senza aspettare.
...
* **Verificare che il backup su Gist sia configurato su Render**
  ...
  da questo o futuri fix applicativi. Ora si configura e si verifica
  direttamente in `/diagnostica` (punto -6), invece di doverlo dedurre
  o farlo a mano su tre siti diversi.
...
* La forma Xiaomi aggiunta a `_MODEL_CODE_SHAPES` (punto -2) copre lo
  ...
  cercare.
* Il modulo "Configura il backup"/"Rifai la configurazione" (punti -6 e
  -7) non può verificare né toccare Render: se il token e il Gist sono
  corretti ma la configurazione su Render è comunque sbagliata (nomi
  delle variabili diversi, servizio Render diverso da quello atteso),
  questa sessione non ha modo di saperlo da qui.
* La priorità europea (punto -8) copre Samsung e OnePlus/OPPO. Xiaomi e
  le fonti non-AER di HONOR/Motorola/realme potrebbero avere lo stesso
  problema ma non sono state toccate per mancanza di un fixture/caso
  reale da verificare — indovinare la regola sarebbe stato peggio di
  non applicarla.

## Consegna

Zip consegnato: `android-updater-fix-12ago-v60.zip` (sostituisce il
v59: aggiunge il punto -8, la priorità alla variante europea nella
ricerca OnePlus/OPPO, richiesta esplicitamente dall'utente).
