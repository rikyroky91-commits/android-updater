...
def _simili_di(request: Request, esito: dict, solo_stesso_software: bool) -> dict:
    ...
    chiavi_rif = [chiave_rif] + [
        extract.device_key(marca_gruppo, forma)
        for n in (nome, titolo_scheda)
        for forma in simili.forme_stesso_telefono(n)]
    chiavi_rif = list(dict.fromkeys(c for c in chiavi_rif if c))
