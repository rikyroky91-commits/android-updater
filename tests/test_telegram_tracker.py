...
class TestOrdineDelleFonti(unittest.TestCase):
    """Dove si colloca il canale rispetto alle altre fonti Oppo."""

    def test_registrata_come_curated_non_structured(self):
        from core import config as C, sources
        fonte = next(s for s in sources.SOURCES if s.key == "oplus_telegram")
        self.assertEqual(fonte.trust, C.TRUST_CURATED)
        self.assertEqual(fonte.brand, C.OPPO)

    def test_viene_dopo_l_archivio_ufficiale_oppo(self):
        """Se Oppo pubblica il firmware di quel modello, vince Oppo."""
        ...
        self.assertLess(etichette.index("archivio firmware ufficiale Oppo"),
                        etichette.index("canale rollout OxygenOS/ColorOS (non ufficiale)"))

    def test_viene_prima_delle_fonti_con_versione_di_fabbrica(self):
        """Una build reale vale più della versione uscita di fabbrica."""
        from core import sources
        etichette = [v.etichetta for v in sources._STRUCTURED_LOOKUPS_LIST]
        posizione = etichette.index("canale rollout OxygenOS/ColorOS (non ufficiale)")
        self.assertLess(posizione, etichette.index("piano ufficiale realme"))
        self.assertLess(posizione, etichette.index("elenco ufficiale Oppo"))
