...
class TestOrdineFonti(unittest.TestCase):
    ...
    def test_prima_del_canale_telegram(self):
        """A parità di trust vince il dato prodotto da uno script su
        firmware reali, non la prosa di una persona."""
        from core import sources
        etichette = [v.etichetta for v in sources._STRUCTURED_LOOKUPS_LIST]
        self.assertLess(etichette.index("tracker ARB OnePlus/OPPO (non ufficiale)"),
