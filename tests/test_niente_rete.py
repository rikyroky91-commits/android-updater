...
from core import scan, soc, sources, specs, storage, versus  # noqa: E402

_FIXTURES = os.path.join(os.path.dirname(os.path.abspath(__file__)), "fixtures")
with open(os.path.join(_FIXTURES, "aer_devices.json"), encoding="utf-8") as _f:
    AER_VOCI = json.load(_f)["items"]

with open(os.path.join(_FIXTURES, "specs_devices.tar.gz"), "rb") as _f:
    SPECS_SCHEDE = specs.leggi_archivio(_f.read())
...
class TestLaRicercaNonEsceInRete(unittest.TestCase):
    ...
    def setUp(self):
        ...
        # I DUE CATALOGHI CHE HANNO UN CLIENT HTTP PROPRIO. Non passano da
        # `sources.http_get`, quindi gli agganci qui sopra non li toccano:
        # `soc` scarica il dataset multi-marca da raw.githubusercontent,
        # `specs` scarica l'archivio delle schede da codeload. Entrambi
        # vengono attraversati da `search_model`, che allega il processore
        # a ogni risultato.
        self._orig_soc_dataset = soc._scarica_dataset
        self._orig_specs_scarica = specs._scarica
        # LA TERZA FONTE CON UN CLIENT HTTP PROPRIO. `versus` è il ripiego
        # di `specs.cerca` per le marche che il mirror GSMArena non ha, e
        # `soc.per_modello` lo attraversa in fondo alla catena: senza questo
        # aggancio ogni ricerca a vuoto su un realme o un HONOR uscirebbe in
        # rete, che è esattamente ciò che questo file esiste per impedire.
        self._orig_versus_scarica = versus._scarica
        soc._scarica_dataset = lambda: None
        specs._scarica = lambda: None
        versus._scarica = lambda url, parametri=None: None
        versus.reset_cache()
        soc.reset_cache()
        specs.carica_da(SPECS_SCHEDE, "fixture di test")
    ...
    def tearDown(self):
        ...
        soc._scarica_dataset = self._orig_soc_dataset
        specs._scarica = self._orig_specs_scarica
        versus._scarica = self._orig_versus_scarica
        versus.reset_cache()
        soc.reset_cache()
        specs.reset_cache()
        modelcodes.reset_cache()
...
class TestGuastoDiReteNonPropagaEccezioni(unittest.TestCase):
    ...
    def test_fonti_senza_rete_restituiscono_un_errore_non_lo_sollevano(self):
        ...
        Senza questo il test non misura il guasto: `fetch_honor_aer` e
        `fetch_vivo_aer` tengono la pagina per un'ora, e se un test
        precedente l'ha scaricata davvero rispondono con quella — items
        pieni ed errore vuoto — cioè falliscono affermando l'opposto di
        quello che verificano. Falliva così da prima che realme avesse una
        cache; ora che ce l'ha anche lei, la regola vale per tutte e tre.

        Non è una comodità: una fonte che risponde dalla cache mentre la
        rete è giù sta dichiarando come fresco un dato che non lo è, ed è
        proprio ciò che questo file esiste per escludere.
        """
        aer_catalog.reset_cache()
        sources.azzera_cache_fonti()
        try:
                ...
                for fonte in (sources.fetch_aer_catalog, sources.fetch_honor_aer,
                    ...
                    with self.subTest(fonte=fonte.__name__):
                        ...
                        self.assertTrue(errore,
                                        "un guasto deve essere rumoroso, non vuoto")
        finally:
            aer_catalog.reset_cache()
            sources.azzera_cache_fonti()
