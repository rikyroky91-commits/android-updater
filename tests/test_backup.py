...
from core import backup, config as C  # noqa: E402
...
class BaseBackup(unittest.TestCase):
    ...
    def _scrivi_db(self, contenuto=b"contenuto del database" * 500):
...
class TestSalvataggio(BaseBackup):
    def test_salva_su_gist_comprimendo(self):
        ...
        # Deve essere davvero compresso e ricostruibile identico.
        self.assertEqual(gzip.decompress(base64.b64decode(caricato)), contenuto)
...
class TestRipristino(BaseBackup):
    ...
    def test_forzare_il_ripristino_e_possibile(self):
        ...
        os.environ["BACKUP_GIST_ID"] = "abc123"
        os.environ["BACKUP_GITHUB_TOKEN"] = "token"
        self._scrivi_db(b"dati locali" * 500)
        atteso = b"copia dall'archivio" * 300
        backup.requests = self._gist_con(atteso)

        ok, _ = backup.ripristina(solo_se_mancante=False)
        self.assertTrue(ok)
        with open(self._db, "rb") as f:
            self.assertEqual(f.read(), atteso)
